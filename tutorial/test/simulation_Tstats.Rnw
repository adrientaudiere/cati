...
\vfill
\begin{center}
\textbf{To read a summary of this appendix, see directly section Summary.}
...
@
  ...
  This document was created with \Sexpr{sessionInfo()[[1]]$version.string} on \Sexpr{Sys.info()[1]}. See below for more information. The speed computations of \texttt{cati} main functions are develop  in the \href{http://sourceforge.net/p/cati-r/code/ci/master/tree/tutorial/vignettes/vignette.pdf}{cati tutorial}.
...
for(n in 1:npermut){#for each permutation
 ...
 colnames(ex.traits1) <- paste("traits", c("a", "b"), sep = " ")
...
<<No_Filter_plots, fig.cap = "Distribution of traits for one randomization without filter", crop = TRUE>>= 
par(mfrow=c(2, 2))
plotDistri(ex.traits1, rep("toutes_sp", times = dim(ex.traits1)[1]), ex.indplot1, 
           plot.ask = F, multipanel = F, leg =c(T, F))
plotDistri(ex.traits1, rep("region", times = dim(ex.traits1)[1]), ex.sp1, 
           plot.ask = F, multipanel = F, leg =c(T, F))
par(mfrow=c(1, 1))
@

<<No_Filter_plots_Tstats, fig.height = 4, fig.cap = "Results of T-statistics for one randomization without filter", crop = TRUE>>= 
plot(res.simu1[[1]])
@
...
<<No_Filter_results, fig.cap = "Local alpha errors: Ordered local p.values", crop = TRUE>>= 
par(mfrow = c(3, 2))
...
<<No_filter_summary_plot, fig.cap = "Global alpha errors: Ordered global Standardized Effect Size", crop = TRUE>>= 
par(mfrow = c(3, 2))
par(mar = c(3, 3.5, 2, 0.2))
...
plot(xx, type = "l", main = "T_IC.IR norm", ylim = c(-3, 3),
     ylab = NA, xlab = NA)
rect(-10,mean(xx_lim.inf), npermut, mean(xx_lim.sup), col = rgb(0, 0, 0, 0.2))
...
@
  ...
  \subsection {Conclusion on alpha error}
...
\hline \hline \hline
\multirow{6}{*}{Trait normally distributed} & \multirow{2}{*}{$T_{IP.IC}$} & no (local) & \Sexpr{nbre_alpha_error_norm_loc_Tipic} \tabularnewline
\cline{3-4} 
 & & yes (global) & \Sexpr{alpha1} \tabularnewline
\cline{2-4} 
 & \multirow{2}{*}{$T_{IC.IR}$} & no (local) & \Sexpr{nbre_alpha_error_norm_loc_Ticir} \tabularnewline
\cline{3-4} 
 & & yes (global) & \Sexpr{alpha3} \tabularnewline
\cline{2-4} 
 & \multirow{2}{*}{$T_{PC.PR}$} & no (local) & \Sexpr{nbre_alpha_error_norm_loc_Tpcpr} \tabularnewline
\cline{3-4}
 & & yes (global)& \Sexpr{alpha5} \tabularnewline

\hline \hline \hline

\multirow{6}{*}{Trait uniformally distributed} & \multirow{2}{*}{$T_{IP.IC}$} & no (local) & \Sexpr{nbre_alpha_error_uni_loc_Tipic} \tabularnewline
\cline{3-4}
 & & yes (global) &  \Sexpr{alpha2} \tabularnewline
\cline{2-4} 
 & \multirow{2}{*}{$T_{IC.IR}$} & no (local) & \Sexpr{nbre_alpha_error_uni_loc_Ticir} \tabularnewline
\cline{3-4} 
 & & yes (global) &  \Sexpr{alpha4} \tabularnewline
\cline{2-4} 
 & \multirow{2}{*}{$T_{PC.PR}$} & no (local) & \Sexpr{nbre_alpha_error_uni_loc_Tpcpr} \tabularnewline
\cline{3-4} 
 & & yes (global)&  \Sexpr{alpha6} \tabularnewline
\hline 
\end{tabular}
...
The results of table 1 attest of a good behavior of the T-statistics regardless of the distributions of the traits. Alpha-error of the local indices are always near the wanted alpha-error of 2.5\%. Logically the global indices which averaged local indices are far more robust. Note that the higher alpha-error is obtain using $T_{PC/PR}$ on the trait normally distributed (\Sexpr{nbre_alpha_error_norm_loc_Tpcpr}).
...
Now we can start the simulation of \Sexpr{nb_param_val} parameters values and \Sexpr{N_repet_Param*length(param_SD)} permutations. 
...
for(n in 1:nperm){#for each permutation
 ...
 ex.indplot2 <- as.factor(rep(com, Nind/Ncom))
...
<<typical_result_internal_filtering1, fig.cap = "Distribution of traits for one randomization with internal filtering", crop = TRUE>>= 
par(mfrow=c(2, 2))
...
<<typical_result_internal_filtering2, fig.height = 8, fig.cap = "Results of T-statistics for two randomization with contrasted strength of internal filtering", crop = TRUE>>= 
par(mfrow=c(2, 1))
plot(res.simu2[[1]])
plot(res.simu2[[n]])
par(mfrow=c(1, 1))
...
<<Internal_filter_results_pval_local, fig.height = 3, crop = TRUE, fig.cap = "Beta-error mixing all strength of internal filtering" >>= 
par(mfrow = c(1, 2))
xx <- log10(sort(unlist(lapply(res.simu2.pval, function(x) x[1:10, 1]))))
plot(xx, type = "l", main = "T_IP.IC_distriNorm", 
...
plot(xx, type = "l", main = "T_IP.IC_distriUni", 
     xlab = "rank", ylab = "Standardized Effect Size")
abline(h = log10(0.05))
nbre_beta_error <- round((sum(xx>log10(0.05))+1)/(length(xx)+1), 4)
text(0, -0.8, labels = paste("beta error", nbre_beta_error, sep = " = "), 
...
<<In_filter_results_plot_initparam, fig.height = 4, fig.cap = "Local $T_{IP/IC}$ SES and initial parameters values: Standardized effect size of $T_{IP/IC}$ in function of the strength of internal filter defined by the ratio of the initial range parameter by the initial standard error parameter.", dev ='png', dpi = 100, fig.scap = "Local $T_{IP/IC}$ SES and initial parameters values">>= 
...
plot(meanSES.2loc.norm_Tipic, init_param.loc, pch = 16, col = rgb(0, 0, 0, 0.6),
     ...
     na.rm = T), 0),
     main = "T_IP.IC: normal in black, uniform in purple",
     xlab = "standardized effect size of T_IP.IC",
     ylab = "initial parameters: mean/sd")
...
colnames(res_beta_Tipic) <- paste("strength", 1:ncol(res_beta_Tipic))
rownames(res_beta_Tipic) <- c("effect of sd (normal)", "effect of sd (uniform)", 
        "effect of range (normal)", "effect of range (uniform)")
@

<<In_resultsof_local_betaerrors>>= 
res_beta_Tipic
@
 

Now we can see the beta-error for each strength. A high value of range and a low value of standard error defined a high strength of filtering. For example, in the case of a trait normally distributed, the proportion of false negative is \Sexpr{round(res_beta_Tipic[1, 1]*100, 2)}\% for the higher value of standard errors and thus for the lower strength of internal filtering.
...
<<In_filter_results_plot_modeledparam, fig.height = 4, fig.cap = "Local $T_{IP/IC}$ SES and modeled parameters values: Standardized effect size of $T_{IP/IC}$ in function of the strength of internal filter defined by the ratio of the modeled range parameter by the modeled standard error parameter.", dev ='png', dpi = 100, fig.scap = "Local $T_{IP/IC}$ SES and modeled parameters values">>= 
...
plot(meanSES.2loc.norm_Tipic, modeled_param_norm.loc, pch = 16, col = rgb(0, 0, 0, 0.6),
     ...
     main = "T_IP.IC: normal in black, uniform in purple",
     xlab = "standardized effect size of T_IP.IC",
     ylab = "modeled parameters: mean/sd")
abline(v = mean(SES.inf.MEAN.norm_Tipic, na.rm = T))
...
There is a very hich correlation coefficient between initial and modeled parameter either for the range (traits a: Pearson correlation = \Sexpr{round(cor.test(param_DIST_between_MEAN, modeled_param_norm)[[4]], 3)}; traits b: Pearson correlation = \Sexpr{round(cor.test(param_DIST_between_MEAN, modeled_param_uni)[[4]], 3)}) and for the standard error(traits a and b: Pearson correlation = \Sexpr{round(cor.test(sd_mean.param, mean_sd_of_com)[[4]], 3)}). Consequently the consistency between the figures \ref{fig:In_filter_results_plot_initparam} and  \ref{fig:In_filter_results_plot_modeledparam} is not suprising. 
...
<<Internal_filter_Results_plots_initial_param, fig.height = 4, fig.cap = "Global $T_{IP/IC}$ SES and initial parameters values: Standardized effect size of $T_{IP/IC}$ in function of the strength of internal filter defined by the ratio of the inital range parameter by the initial standard error parameter.", dev ='png', dpi = 100, fig.scap = "Global $T_{IP/IC}$ SES and initial parameters values">>= 
...
<<Internal_filter_Results_plots_modeled_param, fig.height = 4, fig.cap = "Global $T_{IP/IC}$ SES and modeled parameters values: Standardized effect size of $T_{IP/IC}$ in function of the strength of internal filter defined by the ratio of the modeled range parameter by the modeled standard error parameter.", dev ='png', dpi = 100, fig.scap = "Global $T_{IP/IC}$ SES and modeled parameters values">>= 
mean_dist_sp.interm <- t(matrix(unlist(lapply(mean_dist_sp, function(x) x)), nrow = 2))
mean_sd_of_com <- unlist(lapply(sd.sp_stock2, function(x) mean(x)))
...
plot(meanSES.2loc.norm_Tipic ~ init_param.loc, col = rgb(0, 0, 0, 0.2), 
     ...
     xlab = "initial parameters: mean/sd",
     type = "n")
...
for(n in 1:nperm){#for each permutation
 ...
 ex.indplot3 <- as.factor(rep(com, 100))
 ...
 colnames(ex.traits3) <- paste("traits", c("a", "b"), sep = " ")
...
<<typical_results_external_filtering1, fig.cap = "Distribution of traits for one randomization with external filter", crop = TRUE>>= 
par(mfrow=c(2, 2))
plotDistri(ex.traits3, rep("toutes_sp", times = dim(ex.traits3)[1]), ex.indplot3, 
...
<<typical_results_external_filtering2, fig.height = 8, fig.cap = "Results of T-statistics of traits for two randomization with contrasted strength of external filter", crop = TRUE>>= 
par(mfrow=c(2, 1))
plot(res.simu3[[1]])
plot(res.simu3[[n]])
par(mfrow=c(1, 1))
...
<<Ex_filter_results_plot_initparam1, fig.height = 4, fig.cap = "Local $T_{IC/IR}$ SES and initial parameters values: Standardized effect size of $T_{IC/IR}$ in function of the strength of external filter defined by the ratio of the initial range parameter by the initial standard error parameter.", dev ='png', dpi = 100, fig.scap = "Local $T_{IC/IR}$ SES and initial parameters values">>= 
...
plot(meanSES.3loc.norm_Ticir, init_param.loc, pch = 16, col = rgb(0, 0, 0, 0.6),
     ...
     na.rm = T), 0),
     main = "T_IC.IR: normal in black, uniform in purple",
...
<<Ex_filter_results_plot_initparam2, fig.height = 4, fig.cap = "Local $T_{PC/PR}$ SES and initial parameters values: Standardized effect size of $T_{PC/PR}$ in function of the strength of external filter defined by the ratio of the initial range parameter by the initial standard error parameter.", dev ='png', dpi = 100, fig.scap = "Local $T_{PC/PR}$ SES and initial parameters values">>= 
plot(meanSES.3loc.norm_Tpcpr, init_param.loc, pch = 16, col = rgb(0, 0, 0, 0.6),
     ...
     na.rm = T), 0),
     main = "T_PC.PR: normal in black, uniform in purple",
...
colnames(res_beta_Ticir) <- paste("strength", 1:ncol(res_beta_Ticir))
rownames(res_beta_Ticir) <- c("effect of sd (normal)", "effect of sd (uniform)", 
        "effect of range (normal)", "effect of range (uniform)")

res_beta_Tpcpr <- rbind(rev(beta_sd_norm_Tpcpr), rev(beta_sd_uni_Tpcpr), 
      beta_range_norm_Tpcpr, beta_range_uni_Tpcpr)
colnames(res_beta_Tpcpr) <- paste("strength", 1:ncol(res_beta_Tpcpr))
rownames(res_beta_Tpcpr) <- c("effect of sd (normal)", "effect of sd (uniform)", 
        "effect of range (normal)", "effect of range (uniform)")
@

<<Ex_resultsof_local_betaerrors>>= 
res_beta_Ticir
res_beta_Tpcpr
@
...
<<Ex_filter_results_plot_modeledparam1, fig.height = 4, fig.cap = "Local $T_{IC/IR}$ SES and modeled parameters values: Standardized effect size of $T_{IC/IR}$ in function of the strength of external filter defined by the ratio of the modeled range parameter by the modeled standard error parameter.", dev ='png', dpi = 100, fig.scap = "Local $T_{IC/IR}$ SES and modeled parameters values">>= 
plot(meanSES.3loc.norm_Ticir, modeled_param_norm.loc, pch = 16, col = rgb(0, 0, 0, 0.6),
  xlim = c(min(c(unlist(meanSES.3loc.norm_Ticir), unlist(meanSES.3loc.uni_Ticir)), 
    na.rm = T), 0),
  ylim = c(0, max(c(modeled_param_norm.loc, modeled_param_uni.loc), na.rm = T)),
  main = "T_IC.IR: normal in black, uniform in purple",
  xlab = "standardized effect size of T_IC.IR",
  ylab = "modeled parameters: mean/sd")
abline(v = mean(SES.inf.MEAN.norm_Ticir, na.rm = T))
...
<<Ex_filter_results_plot_modeledparam2, fig.height = 4, fig.cap = "Local $T_{PC/PR}$ SES and modeled parameters values: Standardized effect size of $T_{PC/PR}$ in function of the strength of external filter defined by the ratio of the modeled range parameter by the modeled standard error parameter.", dev ='png', dpi = 100, fig.scap = "Local $T_{PC/PR}$ SES and modeled parameters values">>= 
...
<<External_filter_Results_plots_initial_param1, fig.height = 4, fig.cap = "Global $T_{IC/IR}$ SES and initial parameters values: Standardized effect size of $T_{IC/IR}$ in function of the strength of external filter defined by the ratio of the inital range parameter by the initial standard error parameter.", dev ='png', dpi = 100, fig.scap = "Global $T_{IC/IR}$ SES and initial parameters values">>= 
init_param <- param_range / sd_mean.param
...
<<External_filter_Results_plots_initial_param2, fig.height = 4, fig.cap = "Global $T_{PC/PR}$ SES and initial parameters values: Standardized effect size of $T_{PC/PR}$ in function of the strength of external filter defined by the ratio of the inital range parameter by the initial standard error parameter.", dev ='png', dpi = 100, fig.scap = "Global $T_{PC/PR}$ SES and initial parameters values">>= 
...
<<External_filter_Results_plots_modeled_param1, fig.height = 4, fig.cap = "Global $T_{IC/IR}$ SES and modeled parameters values: Standardized effect size of $T_{IC/IR}$ in function of the strength of external filter defined by the ratio of the modeled range parameter by the modeled standard error parameter.", dev ='png', dpi = 100, fig.scap = "Global $T_{IC/IR}$ SES and modeled parameters values">>= 
mean_range_com.interm <- t(matrix(unlist(lapply(mean_range_com, function(x) x)), nrow = 2))
mean_sd_of_com <- unlist(lapply(sd.com_stock3, function(x) mean(x)))
...
plot(meanSES.3glob.norm_Ticir, modeled_param_norm, pch = 16, 
  ...
  xlim = c(min(c(meanSES.3glob.norm_Ticir, meanSES.3glob.uni_Ticir), 
    na.rm = T), 0),
  ylim = c(0, max(c(modeled_param_norm, modeled_param_uni), na.rm = T)),
  xlab = "standardized effect size of T_IC.IR",
  ylab = "initial parameters: mean/sd")
...
<<External_filter_Results_plots_modeled_param2, fig.height = 4, fig.cap = "Global $T_{PC/PR}$ SES and modeled parameters values: Standardized effect size of $T_{PC/PR}$ in function of the strength of external filter defined by the ratio of the modeled range parameter by the modeled standard error parameter.", dev ='png', dpi = 100, fig.scap = "Global $T_{PC/PR}$ SES and modeled parameters values">>= 
...
plot(meanSES.3loc.norm_Tpcpr ~ init_param.loc, col = rgb(0, 0, 0, 0.2), 
     ...
     main = "T_PC.IR: normal in black, uniform in purple",
...
\section{Internal and external filtering}
  ...
  \subsection{Randomisation outline}
...
ex.indplot4 <- as.factor(rep(com, 100))
...
colnames(ex.traits4) <- paste("traits", c("a", "b"), sep = " ")
...
\hline 
\end{tabular}
\caption{Initial parameter ratio allowing beta-error $<0.05$ for T-statistics}
\end{table}
\end{center}
...
\section{Test of variance decomposition}
...
for(n in 1:nperm){#for each permutation
 ...
 ex.indplot5 <- as.factor(rep(com, 100))
 x2 <- c()
 x3 <- c()
 ex.indplot5 <- as.factor(ex.indplot5 )
 for(c in 1:10){
...
<<Partvar_plot, fig.width = 6, fig.height = 6, fig.cap = "Variance partitioning accross nested scales: each colored lines represent the density of permutations results (sites in blue, species in orange and the remaining variance in brown).">>= 
...
We define several value of turnover strength by deleting individuals from the data set if there their species-components traits and their community-component traits are to different. To be more precise, when \texttt{turnover\_strength} = 1, we delete all individual whose the square of the species value minus the community value is superior o one. 
