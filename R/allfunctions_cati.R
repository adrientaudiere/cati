...
partvar <- function(traits, factors, printprogress = TRUE) {
  ...
  for (t in 1:ntraits) {
    if (sum(is.na(traits[, t])) > 0) {
      ...
      warning(paste(
        "All individuals with one NA (",
        sum(is.na(traits[, t])),
        "individual value(s)) are excluded for the trait",
        t,
        ":",
        namestraits[t],
        sep = " "
    ...
    functionlme = paste(
      'varcomp(lme(trait~1, random = ~1|',
      paste(colnames(factors), collapse = '/'),
      ",na.action = na.omit),1)",
      sep = ""
    )
...
piePartvar <- function(partvar, col.pie = NA, ...) {
  ...
  for (t in 1:ntraits) {
    pie(
      partvar[, t],
      main = colnames(partvar)[t],
      col = col.pie,
      labels = rownames(partvar),
      ...
    )
  }
}
...
Tstats <- function(
  ...
  Tplosp <- unlist(
    strsplit(levels(names_sp_ind.plot), split = "@"),
    use.names = FALSE
  )[2 * (1:nlevels(names_sp_ind.plot))]
  ...
  for (t in 1:ncol(traits)) {
    ...
    for (s in 1:nlevels(ind.plot)) {
      T_IP.IC[s, t] <- mean(
        var_IP[grepl(levels(ind.plot)[s], Tplosp), t],
        na.rm = T
      ) /
        var_IC[s, t]
  ...
  if (is.numeric(nperm)) {
    ...
    if (length(reg.pool) != nlevels(ind.plot)) {
      stop(
        "reg.pool need to be either a matrix or a list of length equal to the number of communities"
      )
    }
    ...
    if (length(SE) != ncol(traits) & length(SE) != 1) {
      stop(
        "The vector SE need to have a length of one or equal to the number of traits"
      )
    }
    ...
    if (!isnullregpool) {
      if (length(SE.reg.pool) != length(reg.pool)) {
        stop(
          "The vector SE.reg.pool need to have the same dimension as reg.pool"
        )
      }
    }
      ...
      for (s in 1:nlevels(ind.plot)) {
        ...
        for (i in 1:nperm) {
          ...
          if (length(traits[ind.plot == levels(ind.plot)[s], t]) != 1) {
            ...
            perm_ind.plot1 <- sample(
              trait.intern[ind.plot == levels(ind.plot)[s]],
              table(ind.plot)[s]
            )
    ...
    for (t in 1:ncol(traits)) {
      ...
      if (printprogress == T) {
        print(paste(round(33.3 + t / ncol(traits) / 3 * 100, 2), "%"))
      } else {}
    }
    ...
    traits_by_sp <- apply(traits, 2, function(x) {
      tapply(x, names_sp_ind.plot, mean)
    })
    traits_by_pop <- traits_by_sp[
      match(names_sp_ind.plot, rownames(traits_by_sp)),
    ]
    ...
    for (t in 1:ncol(traits)) {
      ...
      if (printprogress == T) {
        print(paste(round(66.6 + t / ncol(traits) / 3 * 100, 2), "%"))
      } else {}
    }
    ...
    for (t in 1:ncol(traits)) {
      for (i in 1:nperm) {
        mean_IP_nm2sp[i, t, ] <- tapply(
          unlist(traits.nm2sp[[t]], use.names = FALSE)[
            (1 + (i - 1) * yy):(i * yy)
          ],
          names_sp_ind.plot,
          function(x) mean(x, na.rm = T)
        )
        mean_PC_nm2sp[i, t, ] <- tapply(
          mean_IP_nm2sp[i, t, ],
          Tplosp,
          mean,
          na.rm = T
    ...
    for (t in 1:ncol(traits)) {
      for (i in 1:nperm) {
        var_IP_nm1[i, t, ] <- tapply(
          unlist(traits.nm1[[t]], use.names = FALSE)[
            (1 + (i - 1) * yy):(i * yy)
          ],
          names_sp_ind.plot,
          function(x) var(x, na.rm = T)
        )
        var_PC_nm2sp[i, t, ] <- tapply(
          mean_IP_nm2sp[i, t, ],
          Tplosp,
          var,
          na.rm = T
        )
        var_IC_nm1[i, t, ] <- tapply(
          unlist(traits.nm1[[t]], use.names = FALSE)[
            (1 + (i - 1) * yy):(i * yy)
          ],
          ind.plot,
        ...
        var_IC_nm2[i, t, ] <- tapply(
          unlist(traits.nm2[[t]], use.names = FALSE)[
            (1 + (i - 1) * yy):(i * yy)
          ],
          ind.plot,
          function(x) var(x, na.rm = T)
    ...
    for (t in 1:ncol(traits)) {
      for (i in 1:nperm) {
        for (s in 1:nlevels(ind.plot)) {
          T_IP.IC_nm1[i, t, s] <- mean(
            var_IP_nm1[i, t, grepl(levels(ind.plot)[s], Tplosp)],
            na.rm = T
          ) /
            var_IC_nm1[i, t, s]
...
print.Tstats <- function(x, ...) {
  ...
  sumry <- array(
    "",
    c(TABDIM, 4),
    list(1:TABDIM, c("data", "class", "dim", "content"))
  )
  sumry[1, ] <- c(
    "$traits",
    class(x$traits)[1],
    paste(dim(x$traits)[1], dim(x$traits)[2], sep = ","),
    "traits data"
  )
  sumry[2, ] <- c(
    "$ind.plot",
  ...
  sumry[3, ] <- c(
    ...
    class(x$sp),
    length(x$sp),
    "individuals' groups (e.g. species)"
  )
...
print.ComIndex <- function(x, ...) {
  ...
  if (!is.null(x$null)) {
    cat("\n###############")
    cat(
      "\n$null: list of null values, number of permutations:",
      dim(x$null[[1]])[3],
      "\n"
    )
    cat(
      paste(
        paste("\t$", names(x$list.index)[-seq.interm], sep = ""),
        paste("null model", "=", x$nullmodels, sep = " "),
        sep = " ... "
      ),
      sep = "\n"
    )
...
#' @exportS3Method
plot.Tstats <- function(
  x,
  type = "normal",
  col.index = c("red", "purple", "olivedrab3"),
  add.conf = TRUE,
  color.cond = TRUE,
  ...
  type <- match.arg(
    type,
    c("simple", "simple_range", "normal", "barplot", "bysites", "bytraits")
  )
  ...
  if (length(x$Tstats) != 6) {
    stop(
      "The object x of class Tstats does not contain null value from null models."
    )
  }

  plot(
    as.listofindex(x, namesindex = c("T_IP.IC", "T_IC.IR", "T_PC.PR")),
    type = type,
    col.index = col.index,
    add.conf = add.conf,
...
sum_Tstats <- function(x, val.quant = c(0.025, 0.975), type = "all") {
  ...
  #________________________________________
  ses.T_IP.IC <- (Tst$T_IP.IC -
    apply(Tst$T_IP.IC_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
    apply(Tst$T_IP.IC_nm, c(3, 2), function(x) sd(x, na.rm = T))
  ses.T_IC.IR <- (Tst$T_IC.IR -
    apply(Tst$T_IC.IR_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
    apply(Tst$T_IC.IR_nm, c(3, 2), function(x) sd(x, na.rm = T))
  ses.T_PC.PR <- (Tst$T_PC.PR -
    apply(Tst$T_PC.PR_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
    apply(Tst$T_PC.PR_nm, c(3, 2), function(x) sd(x, na.rm = T))

  ses.T_IP.IC.inf <- (apply(Tst$T_IP.IC_nm, c(3, 2), function(x) {
    quantile(x, na.rm = T, prob = val.quant[1])
  }) -
    apply(Tst$T_IP.IC_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
    apply(Tst$T_IP.IC_nm, c(3, 2), function(x) sd(x, na.rm = T))
  ses.T_IC.IR.inf <- (apply(Tst$T_IC.IR_nm, c(3, 2), function(x) {
    quantile(x, na.rm = T, prob = val.quant[1])
  }) -
    apply(Tst$T_IC.IR_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
    apply(Tst$T_IC.IR_nm, c(3, 2), function(x) sd(x, na.rm = T))
  ses.T_PC.PR.inf <- (apply(Tst$T_PC.PR_nm, c(3, 2), function(x) {
    quantile(x, na.rm = T, prob = val.quant[1])
  }) -
    apply(Tst$T_PC.PR_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
    apply(Tst$T_PC.PR_nm, c(3, 2), function(x) sd(x, na.rm = T))

  ses.T_IP.IC.sup <- (apply(Tst$T_IP.IC_nm, c(3, 2), function(x) {
    quantile(x, na.rm = T, prob = val.quant[2])
  }) -
    apply(Tst$T_IP.IC_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
    apply(Tst$T_IP.IC_nm, c(3, 2), function(x) sd(x, na.rm = T))
  ses.T_IC.IR.sup <- (apply(Tst$T_IC.IR_nm, c(3, 2), function(x) {
    quantile(x, na.rm = T, prob = val.quant[2])
  }) -
    apply(Tst$T_IC.IR_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
    apply(Tst$T_IC.IR_nm, c(3, 2), function(x) sd(x, na.rm = T))
  ses.T_PC.PR.sup <- (apply(Tst$T_PC.PR_nm, c(3, 2), function(x) {
    quantile(x, na.rm = T, prob = val.quant[2])
  }) -
    apply(Tst$T_PC.PR_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
    apply(Tst$T_PC.PR_nm, c(3, 2), function(x) sd(x, na.rm = T))

  ses.T_IP.IC.mean <- t(colMeans(
    (Tst$T_IP.IC -
      apply(Tst$T_IP.IC_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
      apply(Tst$T_IP.IC_nm, c(3, 2), function(x) sd(x, na.rm = T)),
    na.rm = T
  ))
  ses.T_IC.IR.mean <- t(colMeans(
    (Tst$T_IC.IR -
      apply(Tst$T_IC.IR_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
      apply(Tst$T_IC.IR_nm, c(3, 2), function(x) sd(x, na.rm = T)),
    na.rm = T
  ))
  ses.T_PC.PR.mean <- t(colMeans(
    (Tst$T_PC.PR -
      apply(Tst$T_PC.PR_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
      apply(Tst$T_PC.PR_nm, c(3, 2), function(x) sd(x, na.rm = T)),
    na.rm = T
  ...
  if (type == "all" | type == "p.value") {
    ...
    for (t in 1:ncol(res$Tstats$T_IP.IC)) {
      for (s in 1:nrow(res$Tstats$T_IP.IC)) {
        p.valueT_IP.IC.sup[s, t] <- (sum(
          res$Tstats$T_IP.IC[s, t] < res$Tstats$T_IP.IC_nm[, t, s],
          na.rm = T
        ) +
          1) /
          (1 + length(res$Tstats$T_IP.IC_nm[, t, s]))
        p.valueT_IC.IR.sup[s, t] <- (sum(
          res$Tstats$T_IC.IR[s, t] < res$Tstats$T_IC.IR_nm[, t, s],
          na.rm = T
        ) +
          1) /
          (1 + length(res$Tstats$T_IC.IR_nm[, t, s]))
        p.valueT_PC.PR.sup[s, t] <- (sum(
          res$Tstats$T_PC.PR[s, t] < res$Tstats$T_PC.PR_nm[, t, s],
          na.rm = T
        ) +
          1) /
          (1 + length(res$Tstats$T_PC.PR_nm[, t, s]))

        p.valueT_IP.IC.inf[s, t] <- (sum(
          res$Tstats$T_IP.IC[s, t] > res$Tstats$T_IP.IC_nm[, t, s],
          na.rm = T
        ) +
          1) /
          (1 + length(res$Tstats$T_IP.IC_nm[, t, s]))
        p.valueT_IC.IR.inf[s, t] <- (sum(
          res$Tstats$T_IC.IR[s, t] > res$Tstats$T_IC.IR_nm[, t, s],
          na.rm = T
        ) +
          1) /
          (1 + length(res$Tstats$T_IC.IR_nm[, t, s]))
        p.valueT_PC.PR.inf[s, t] <- (sum(
          res$Tstats$T_PC.PR[s, t] > res$Tstats$T_PC.PR_nm[, t, s],
          na.rm = T
        ) +
          1) /
  ...
  if (type == "binary") {
    summ.Tstats <- matrix(
      "H0 not rejected",
      nrow = 6,
      ncol = dim(cond.T_IP.IC.inf)[2]
    )
    summ.Tstats <- rbind(
      cond.T_IP.IC.inf.mean,
      cond.T_IP.IC.sup.mean,
      cond.T_IC.IR.inf.mean,
      cond.T_IC.IR.sup.mean,
      cond.T_PC.PR.inf.mean,
      cond.T_IC.IR.sup.mean
    )
    rownames(summ.Tstats) <- c(
      "T_IP.IC.inf",
      "T_IP.IC.sup",
      "T_IC.IR.inf",
      "T_IC.IR.sup",
    ...
    summ.Tstats[1, ] <- paste(
      round(
        colSums(cond.T_IP.IC.inf, na.rm = T) /
          colSums(!is.na(cond.T_IP.IC.inf)),
        2
      ) *
        100,
      "%",
    ...
    summ.Tstats[2, ] <- paste(
      round(
        colSums(cond.T_IP.IC.sup, na.rm = T) /
          colSums(!is.na(cond.T_IP.IC.sup)),
        2
      ) *
        100,
      "%",
    ...
    summ.Tstats[3, ] <- paste(
      round(
        colSums(cond.T_IC.IR.inf, na.rm = T) /
          colSums(!is.na(cond.T_IC.IR.inf)),
        2
      ) *
        100,
      "%",
    ...
    summ.Tstats[4, ] <- paste(
      round(
        colSums(cond.T_IC.IR.sup, na.rm = T) /
          colSums(!is.na(cond.T_IC.IR.sup)),
        2
      ) *
        100,
      "%",
    ...
    summ.Tstats[5, ] <- paste(
      round(
        colSums(cond.T_PC.PR.inf, na.rm = T) /
          colSums(!is.na(cond.T_PC.PR.inf)),
        2
      ) *
        100,
      "%",
    ...
    summ.Tstats[6, ] <- paste(
      round(
        colSums(cond.T_PC.PR.sup, na.rm = T) /
          colSums(!is.na(cond.T_PC.PR.sup)),
        2
      ) *
        100,
      "%",
    ...
    summ.Tstats[1, ][which(cond.T_IP.IC.inf.mean)] <- paste(
      round(
        colSums(cond.T_IP.IC.inf, na.rm = T) /
          colSums(!is.na(cond.T_IP.IC.inf)),
        2
      )[which(cond.T_IP.IC.inf.mean)] *
        100,
      "%",
      "*",
      sep = ""
    )
    summ.Tstats[2, ][which(cond.T_IP.IC.sup.mean)] <- paste(
      round(
        colSums(cond.T_IP.IC.sup, na.rm = T) /
          colSums(!is.na(cond.T_IP.IC.sup)),
        2
      )[which(cond.T_IP.IC.sup.mean)] *
        100,
      "%",
      "*",
      sep = ""
    )
    summ.Tstats[3, ][which(cond.T_IC.IR.inf.mean)] <- paste(
      round(
        colSums(cond.T_IC.IR.inf, na.rm = T) /
          colSums(!is.na(cond.T_IC.IR.inf)),
        2
      )[which(cond.T_IC.IR.inf.mean)] *
        100,
      "%",
      "*",
      sep = ""
    )
    summ.Tstats[4, ][which(cond.T_IC.IR.sup.mean)] <- paste(
      round(
        colSums(cond.T_IC.IR.sup, na.rm = T) /
          colSums(!is.na(cond.T_IC.IR.sup)),
        2
      )[which(cond.T_IC.IR.sup.mean)] *
        100,
      "%",
      "*",
      sep = ""
    )
    summ.Tstats[5, ][which(cond.T_PC.PR.inf.mean)] <- paste(
      round(
        colSums(cond.T_PC.PR.inf, na.rm = T) /
          colSums(!is.na(cond.T_PC.PR.inf)),
        2
      )[which(cond.T_PC.PR.inf.mean)] *
        100,
      "%",
      "*",
      sep = ""
    )
    summ.Tstats[6, ][which(cond.T_PC.PR.sup.mean)] <- paste(
      round(
        colSums(cond.T_PC.PR.sup, na.rm = T) /
          colSums(!is.na(cond.T_PC.PR.sup)),
        2
      )[which(cond.T_PC.PR.sup.mean)] *
        100,
      "%",
      "*",
      sep = ""
    ...
    for (t in 1:dim(cond.T_IP.IC.inf)[2]) {
      if (sum(cond.T_IP.IC.inf[, t], na.rm = T) > 0) {
        summ.Tstats[1, t] <- paste(
          na.exclude(rownames(cond.T_IP.IC.inf)[cond.T_IP.IC.inf[, t]]),
          collapse = " "
        )
      } else {
        summ.Tstats[1, t] <- "H0 not rejected"
      }

      if (sum(cond.T_IP.IC.sup[, t], na.rm = T) > 0) {
        summ.Tstats[2, t] <- paste(
          na.exclude(rownames(cond.T_IP.IC.sup)[cond.T_IP.IC.sup[, t]]),
          collapse = " "
        )
      } else {
        summ.Tstats[2, t] <- "H0 not rejected"
      }

      if (sum(cond.T_IC.IR.inf[, t], na.rm = T) > 0) {
        summ.Tstats[3, t] <- paste(
          na.exclude(rownames(cond.T_IC.IR.inf)[cond.T_IP.IC.inf[, t]]),
          collapse = " "
        )
      } else {
        summ.Tstats[3, t] <- "H0 not rejected"
      }

      if (sum(cond.T_IC.IR.sup[, t], na.rm = T) > 0) {
        summ.Tstats[4, t] <- paste(
          na.exclude(rownames(cond.T_IC.IR.sup)[cond.T_IC.IR.sup[, t]]),
          collapse = " "
        )
      } else {
        summ.Tstats[4, t] <- "H0 not rejected"
      }

      if (sum(cond.T_PC.PR.inf[, t], na.rm = T) > 0) {
        summ.Tstats[5, t] <- paste(
          na.exclude(rownames(cond.T_PC.PR.inf)[cond.T_PC.PR.inf[, t]]),
          collapse = " "
        )
      } else {
        summ.Tstats[5, t] <- "H0 not rejected"
      }

      if (sum(cond.T_PC.PR.sup[, t], na.rm = T) > 0) {
        summ.Tstats[6, t] <- paste(
          na.exclude(rownames(cond.T_PC.PR.sup)[cond.T_PC.PR.sup[, t]]),
          collapse = " "
        )
      } else {
        summ.Tstats[6, t] <- "H0 not rejected"
      }
    }
    rownames(summ.Tstats) <- c(
      "T_IP.IC.inf",
    ...
    rownames(summ.Tstats) <- c(
      paste(rep("T_IP.IC.inf", dim(Tst$T_IP.IC)[1]), rownames(Tst$T_IP.IC)),
      paste(rep("T_IP.IC.sup", dim(Tst$T_IP.IC)[1]), rownames(Tst$T_IP.IC)),
      paste(rep("T_IC.IR.inf", dim(Tst$T_IP.IC)[1]), rownames(Tst$T_IP.IC)),
      paste(rep("T_IC.IR.sup", dim(Tst$T_IP.IC)[1]), rownames(Tst$T_IP.IC)),
      paste(rep("T_PC.PR.inf", dim(Tst$T_IP.IC)[1]), rownames(Tst$T_IP.IC)),
      paste(rep("T_PC.PR.sup", dim(Tst$T_IP.IC)[1]), rownames(Tst$T_IP.IC))
    )
    colnames(summ.Tstats) <- colnames(Tst$T_IP.IC)
  } else if (type == "all") {
    #________________________________________
    ...
    #__________
    ##p.value
    summ.Tstats$p.value <- matrix(
      "H0 not rejected",
      nrow = 6,
      ncol = dim(cond.T_IP.IC.inf)[2]
    )
    summ.Tstats$p.value <- rbind(
      pval$T_IP.IC.inf,
      pval$T_IP.IC.sup,
      pval$T_IC.IR.inf,
      pval$T_IC.IR.sup,
    ...
    #__________
    ##percent
    summ.Tstats$percent <- matrix(
      "H0 not rejected",
      nrow = 6,
      ncol = dim(cond.T_IP.IC.inf)[2]
    )

    summ.Tstats$percent[1, ] <- paste(
      round(
        colSums(cond.T_IP.IC.inf, na.rm = T) /
          colSums(!is.na(cond.T_IP.IC.inf)),
        2
    ...
    summ.Tstats$percent[1, ][which(cond.T_IP.IC.inf.mean)] <- paste(
      round(
        colSums(cond.T_IP.IC.inf, na.rm = T) /
          colSums(!is.na(cond.T_IP.IC.inf)),
        2
    ...
    summ.Tstats$percent[2, ][which(cond.T_IP.IC.sup.mean)] <- paste(
      round(
        colSums(cond.T_IP.IC.sup, na.rm = T) /
          colSums(!is.na(cond.T_IP.IC.sup)),
        2
    ...
    summ.Tstats$percent[3, ][which(cond.T_IC.IR.inf.mean)] <- paste(
      round(
        colSums(cond.T_IC.IR.inf, na.rm = T) /
          colSums(!is.na(cond.T_IC.IR.inf)),
        2
    ...
    summ.Tstats$percent[4, ][which(cond.T_IC.IR.sup.mean)] <- paste(
      round(
        colSums(cond.T_IC.IR.sup, na.rm = T) /
          colSums(!is.na(cond.T_IC.IR.sup)),
        2
    ...
    summ.Tstats$percent[5, ][which(cond.T_PC.PR.inf.mean)] <- paste(
      round(
        colSums(cond.T_PC.PR.inf, na.rm = T) /
          colSums(!is.na(cond.T_PC.PR.inf)),
        2
    ...
    summ.Tstats$percent[6, ][which(cond.T_PC.PR.sup.mean)] <- paste(
      round(
        colSums(cond.T_PC.PR.sup, na.rm = T) /
          colSums(!is.na(cond.T_PC.PR.sup)),
        2
    ...
    for (t in 1:dim(cond.T_IP.IC.inf)[2]) {
      if (sum(cond.T_IP.IC.inf[, t], na.rm = T) > 0) {
        summ.Tstats$sites[1, t] <- paste(
          na.exclude(rownames(cond.T_IP.IC.inf)[cond.T_IP.IC.inf[, t]]),
          collapse = " "
        )
      } else {
        summ.Tstats$sites[1, t] <- "H0 not rejected"
      }

      if (sum(cond.T_IP.IC.sup[, t], na.rm = T) > 0) {
        summ.Tstats$sites[2, t] <- paste(
          na.exclude(rownames(cond.T_IP.IC.sup)[cond.T_IP.IC.sup[, t]]),
          collapse = " "
        )
      } else {
        summ.Tstats$sites[2, t] <- "H0 not rejected"
      }

      if (sum(cond.T_IC.IR.inf[, t], na.rm = T) > 0) {
        summ.Tstats$sites[3, t] <- paste(
          na.exclude(rownames(cond.T_IC.IR.inf)[cond.T_IP.IC.inf[, t]]),
          collapse = " "
        )
      } else {
        summ.Tstats$sites[3, t] <- "H0 not rejected"
      }

      if (sum(cond.T_IC.IR.sup[, t], na.rm = T) > 0) {
        summ.Tstats$sites[4, t] <- paste(
          na.exclude(rownames(cond.T_IC.IR.sup)[cond.T_IC.IR.sup[, t]]),
          collapse = " "
        )
      } else {
        summ.Tstats$sites[4, t] <- "H0 not rejected"
      }

      if (sum(cond.T_PC.PR.inf[, t], na.rm = T) > 0) {
        summ.Tstats$sites[5, t] <- paste(
          na.exclude(rownames(cond.T_PC.PR.inf)[cond.T_PC.PR.inf[, t]]),
          collapse = " "
        )
      } else {
        summ.Tstats$sites[5, t] <- "H0 not rejected"
      }

      if (sum(cond.T_PC.PR.sup[, t], na.rm = T) > 0) {
        summ.Tstats$sites[6, t] <- paste(
          na.exclude(rownames(cond.T_PC.PR.sup)[cond.T_PC.PR.sup[, t]]),
          collapse = " "
        )
      } else {
        summ.Tstats$sites[6, t] <- "H0 not rejected"
      }
    }

    #__________
    ##binary
    summ.Tstats$binary <- matrix(
      "H0 not rejected",
      nrow = 6,
      ncol = dim(cond.T_IP.IC.inf)[2]
    )
    summ.Tstats$binary <- rbind(
      cond.T_IP.IC.inf.mean,
      cond.T_IP.IC.sup.mean,
      cond.T_IC.IR.inf.mean,
      cond.T_IC.IR.sup.mean,
    ...
    #__________
    rownames(summ.Tstats$p.value) <- c(
      rep("T_IP.IC.inf", dim(Tst$T_IP.IC)[1]),
      rep("T_IP.IC.sup", dim(Tst$T_IP.IC)[1]),
      rep("T_IC.IR.inf", dim(Tst$T_IP.IC)[1]),
      rep("T_IC.IR.sup", dim(Tst$T_IP.IC)[1]),
      rep("T_PC.PR.inf", dim(Tst$T_IP.IC)[1]),
      rep("T_PC.PR.sup", dim(Tst$T_IP.IC)[1])
    )
    rownames(summ.Tstats$binary) <- c(
      "T_IP.IC.inf",
      "T_IP.IC.sup",
      "T_IC.IR.inf",
      "T_IC.IR.sup",
    ...
    rownames(summ.Tstats$percent) <- c(
      "T_IP.IC.inf",
      "T_IP.IC.sup",
      "T_IC.IR.inf",
      "T_IC.IR.sup",
    ...
    stop("Error: type must be 'binary', 'percent', 'p.value', 'site' or 'all'.")
  }
...
#' @exportS3Method
barplot.Tstats <- function(
  height,
  val.quant = c(0.025, 0.975),
  col.index = c("red", "purple", "olivedrab3", "white"),
  ylim = NULL,
  ...
) {
  Tst <- height$Tstats

  T_IP.IC.inf <- apply(Tst$T_IP.IC_nm, c(3, 2), function(x) {
    quantile(x, na.rm = T, prob = val.quant[1])
  })
  T_IC.IR.inf <- apply(Tst$T_IC.IR_nm, c(3, 2), function(x) {
    quantile(x, na.rm = T, prob = val.quant[1])
  })
  T_PC.PR.inf <- apply(Tst$T_PC.PR_nm, c(3, 2), function(x) {
    quantile(x, na.rm = T, prob = val.quant[1])
  })

  T_IP.IC.sup <- apply(Tst$T_IP.IC_nm, c(3, 2), function(x) {
    quantile(x, na.rm = T, prob = val.quant[2])
  })
  T_IC.IR.sup <- apply(Tst$T_IC.IR_nm, c(3, 2), function(x) {
    quantile(x, na.rm = T, prob = val.quant[2])
  })
  T_PC.PR.sup <- apply(Tst$T_PC.PR_nm, c(3, 2), function(x) {
    quantile(x, na.rm = T, prob = val.quant[2])
  })
    ...
    ylim = c(
      min(
        c(
          ...
          T_IC.IR.inf,
          T_PC.PR.inf,
          colMeans(na.omit(Tst$T_IP.IC)) - apply(na.omit(Tst$T_IP.IC), 2, sd),
          colMeans(na.omit(Tst$T_IC.IR)) - apply(na.omit(Tst$T_IC.IR), 2, sd),
          colMeans(na.omit(Tst$T_PC.PR)) - apply(na.omit(Tst$T_PC.PR), 2, sd)
        ),
        na.rm = T
      ),
      max(
  ...
  df.bar <- barplot(
    rbind(
      colMeans(na.omit(Tst$T_IP.IC)),
      colMeans(na.omit(Tst$T_IC.IR)),
      colMeans(na.omit(Tst$T_PC.PR)),
      0
    ),
    beside = T,
    plot = F
  ...
  points(
    type = "l",
    df.bar[1, ],
    colMeans(T_IP.IC.sup, na.rm = T),
    col = col.index[1]
  )
  points(
    type = "l",
    df.bar[2, ],
    colMeans(T_IC.IR.sup, na.rm = T),
    col = col.index[2]
  )
  points(
    type = "l",
    df.bar[3, ],
    colMeans(T_PC.PR.sup, na.rm = T),
    col = col.index[3]
  )
  ...
  points(
    type = "l",
    df.bar[1, ],
    colMeans(T_IP.IC.inf, na.rm = T),
    col = col.index[1]
  )
  points(
    type = "l",
    df.bar[2, ],
    colMeans(T_IC.IR.inf, na.rm = T),
    col = col.index[2]
  )
  points(
    type = "l",
    df.bar[3, ],
    colMeans(T_PC.PR.inf, na.rm = T),
    col = col.index[3]
  )
}
...
ComIndex <- function(
  ...
  if (is.null(index)) {
    stop(
      "There is no default index to compute. Please add metrics using the 'index' argument."
    )
  }
  ...
  if (is.null(ind.plot)) {
    if (is.null(com)) {
      stop("Need 'ind.plot' or 'com' argument")
    }
  ...
  if (!is.null(ind.plot) & !is.null(com)) {
    warning(
      "If ind.plot and com are provide, the function use only the argument ind.plot"
    )
  }
  ...
  if (!is.null(nullmodels) & !is.null(nperm)) {
    ...
    nullmodels <- match.arg(
      nullmodels,
      c("local", "regional.ind", "regional.pop", "regional.pop.prab"),
      several.ok = TRUE
    )
  ...
  if (is.numeric(nperm)) {
    ...
    if (!is.null(reg.pool) & sum(nullmodels == "regional.ind") == 0) {
      warning(
        "Custom regional pool 'reg.pool' is only used in the case of null model regional.ind"
      )
    }
    ...
    if (sum(nullmodels == "1") > 0) {
      ...
      for (t in 1:ntr) {
        traits.nm1[[eval(namestraits[t])]] <- matrix(
          NA,
          nrow = dim(traits)[1],
          ncol = nperm
        )
        ...
        for (n in 1:nperm) {
          for (s in 1:ncom) {
            ...
            perm_ind.plot[[s]] <- sample(
              traits[ind.plot == levels(ind.plot)[s], t],
              table(ind.plot)[s]
            )
          }

          traits.nm1[[eval(namestraits[t])]][, n] <- unlist(
            perm_ind.plot,
            use.names = FALSE
          )
        }
    ...
    if (sum(nullmodels == "2") > 0) {
      ...
      for (t in 1:ntr) {
        traits.nm2[[eval(namestraits[t])]] <- matrix(
          NA,
          nrow = dim(traits)[1],
          ncol = nperm
        )
        ...
        for (n in 1:nperm) {
          ...
          traits.nm2[[eval(namestraits[t])]][, n] <- unlist(
            perm_ind.plot,
            use.names = FALSE
          )
        }
    ...
    if (sum(nullmodels == "2sp") > 0) {
      ...
      traits_by_pop <- traits_by_sp[
        match(name_sp_sites, rownames(traits_by_sp)),
      ]

      for (t in 1:ntr) {
        traits.nm2sp[[eval(namestraits[t])]] <- matrix(
          NA,
          nrow = dim(traits)[1],
          ncol = nperm
        )
        ...
        for (n in 1:nperm) {
          ...
          traits.nm2sp[[eval(namestraits[t])]][, n] <- unlist(
            perm_ind.plot,
            use.names = FALSE
          )
        }
    ...
    if (sum(nullmodels == "2sp.prab") > 0) {
      ...
      for (t in 1:ntr) {
        traits.nm2sp.prab[[eval(namestraits[t])]] <- matrix(
          NA,
          nrow = dim(traits_by_sp)[1],
          ncol = nperm
        )
        perm_ind.plot <- list()
        ...
        for (n in 1:nperm) {
          ...
          traits.nm2sp.prab[[eval(namestraits[t])]][, n] <- unlist(
            perm_ind.plot,
            use.names = FALSE
          )
        }
    ...
    if (printprogress == T) {
      print("calculation of null values using null models")
    }

    for (i in 1:nindex) {
      if (nullmodels[i] == "1") {
        nm.bis <- traits.nm1[[1]]
      } else if (nullmodels[i] == "2") {
        nm.bis <- traits.nm2[[1]]
      } else if (nullmodels[i] == "2sp") {
        nm.bis <- traits.nm2sp[[1]]
      } else if (nullmodels[i] == "2sp.prab") {
        nm.bis <- traits.nm2sp.prab[[1]]
      } else {
        print(
          "nullmodels need values local, regional.ind, regional.pop, regional.pop.prab"
        )
      }
      ...
      if (nullmodels[i] == "2sp") {
        nm_bypop.bis[[eval(namesindex[i])]] <- apply(nm.bis, 2, function(x) {
          tapply(x, name_sp_sites, mean, na.rm = T)
        })

        dim2 <- dim(apply(nm_bypop.bis[[eval(namesindex[i])]], 2, function(x) {
          eval(parse(text = functionindex))
        }))[1]
        ...
        nm_bypop.bis[[eval(namesindex[i])]] <- apply(nm.bis, 2, function(x) {
          tapply(x, rownames(traits_by_sp), mean, na.rm = T)
        })
        ...
        dim2 <- dim(apply(nm.bis, 2, function(x) {
          eval(parse(text = functionindex))
        }))[1]
        null[[eval(namesindex[i])]] <- array(NA, dim = c(ntr, dim2, nperm))
        if (is.null(dim2)) {
      ...
      for (t in 1:ntr) {
        ...
        if (nullmodels[i] == "2sp") {
          nm_bypop[[eval(namesindex[i])]] <- apply(nm, 2, function(x) {
            tapply(x, name_sp_sites, mean, na.rm = T)
          })

          null[[eval(namesindex[i])]][t, , ] <- apply(
            nm_bypop[[eval(namesindex[i])]],
            2,
            function(x) eval(parse(text = functionindex))
          )
        } else if (nullmodels[i] == "2sp.prab") {
          nm_bypop[[eval(namesindex[i])]] <- apply(nm, 2, function(x) {
            tapply(x, rownames(traits_by_sp), mean, na.rm = T)
          ...
          null[[eval(namesindex[i])]][t, , ] <- apply(nm, 2, function(x) {
            eval(parse(text = functionindex))
          })
        }
  ...
  if (printprogress == T) {
    print("calculation of observed values")
  }
  ...
  for (i in 1:nindex) {
    ...
    if (is.null(nullmodels)) {
      obs[[eval(namesindex[i])]] <- array(
        dim = c(
          ntr,
          dim(apply(traits, 2, function(x) eval(parse(text = functionindex))))[
            1
          ]
        )
      )
      obs[[eval(namesindex[i])]] <- apply(traits, 2, function(x) {
        eval(parse(text = functionindex))
      })
      #obs[[eval(namesindex[i])]] [ !is.finite(obs[[eval(namesindex[i])]] )] <- NA
    } else if (!is.null(nullmodels)) {
      if (nullmodels[i] == "2sp") {
        traits_by_pop <- apply(traits, 2, function(x) {
          tapply(x, name_sp_sites, mean, na.rm = T)
        })
        obs[[eval(namesindex[i])]] <- array(
          dim = c(
            ntr,
            dim(apply(traits_by_pop, 2, function(x) {
              eval(parse(text = functionindex))
            }))[1]
          )
        )
        obs[[eval(namesindex[i])]] <- apply(traits_by_pop, 2, function(x) {
          eval(parse(text = functionindex))
        })
      }
      ...
      if (nullmodels[i] == "2sp.prab") {
        ...
        obs[[eval(namesindex[i])]] <- array(
          dim = c(
            ntr,
            dim(apply(traits_by_sp, 2, function(x) {
              eval(parse(text = functionindex))
            }))[1]
          )
        )
        obs[[eval(namesindex[i])]] <- apply(traits_by_sp, 2, function(x) {
          eval(parse(text = functionindex))
        })
  ...
  for (i in 1:nindex) {
    ...
    if (is.numeric(nperm)) {
      ...
      name.ComIndex_list.index[seq(1, nindex * 2, by = 2)[i] + 1] <- paste(
        names(null)[i],
        "nm",
        sep = "_"
      )
...
ComIndexMulti <- function(
  ...
  if (is.null(by.factor)) {
    by.factor = rep("Region", length(ind.plot))
  }
  ...
  if (is.numeric(nperm)) {
    ...
    for (i in 1:nindex) {
      if (nullmodels[i] == "1") {
        nm <- array(
          unlist(traits.nm1, use.names = FALSE),
          dim = c(ncol(traits), dim(traits)[1], nperm)
        )
      } else if (nullmodels[i] == "2") {
        nm <- array(
          unlist(traits.nm2, use.names = FALSE),
          dim = c(ncol(traits), dim(traits)[1], nperm)
        )
      } else if (nullmodels[i] == "2sp") {
        nm <- array(
          unlist(traits.nm2sp, use.names = FALSE),
          dim = c(ncol(traits), dim(traits)[1], nperm)
        )
      } else if (nullmodels[i] == "2sp.prab") {
        nm <- array(
          unlist(traits.nm2sp.prab, use.names = FALSE),
          dim = c(ncol(traits), dim(traits_by_sp)[1], nperm)
        )
      } else {
        print("nullmodels need 1, 2, 2sp or 2sp.prab")
      }
      ...
      dim2 <- dim(by(t(nm_n), by.factor, function(x) {
        eval(parse(text = functionindex))
      }))[1]
      ...
      for (n in 1:nperm) {
        null[[eval(namesindex[i])]][, n] <- as.vector(by(
          t(nm[,, n]),
          by.factor,
          function(x) eval(parse(text = functionindex))
        ))
  ...
  for (i in 1:nindex) {
    ...
    dim2 <- dim(by(traits, by.factor, function(x) {
      eval(parse(text = functionindex))
    }))[1]
    obs[[eval(namesindex[i])]] <- rep(NA, times = dim2)

    if (is.null(nullmodels)) {
      obs[[eval(namesindex[i])]] <- as.vector(by(
        traits,
        by.factor,
        function(x) eval(parse(text = functionindex))
      ))
...
as.listofindex <- function(x, namesindex = NULL) {
  ...
  for (i in 1:nlist) {
    if (inherits(x[[i]], "Tstats")) {
      ...
      inherits(x[[i]], "ComIndex") | inherits(x[[i]], "ComIndexMulti")
    ) {
      nindex[i] <- length(x[[i]]$obs)
    } else {
      stop(
        "x must be a list of objects of class Tstats, ComIndex or ComIndexMulti"
      )
    }
  }
  ...
  if (is.null(namesindex)) {
    for (l in 1:nlist) {
      namesindex <- c(
        namesindex,
        paste(rep("index", nindex[l]), l, 1:nindex[l], sep = "_")
      )
    }
  }
...
plot.listofindex <- function(
  ...
  if (!inherits(x, "listofindex")) {
    if (
      inherits(x[[1]], "Tstats") |
        inherits(x[[2]], "ComIndex") |
        inherits(x[[3]], "ComIndexMulti")
    ) {
      x <- as.listofindex(x)
    } else {
      stop(
        "x must be a list of objects of class Tstats, ComIndex, ComIndexMulti or listofindex"
      )
    }
  }
  ...
  if (is.null(namescommunity)) {
    warning(
      "rownames of index.list[[1]] is empty so names of plots cannot be plot"
    )
  }
  if (is.null(namestraits)) {
    warning(
  ...
  if (is.null(ncom)) {
    ncom <- dim(as.matrix(index.list[[1]]))[1]
  }
  if (is.null(ntr)) {
    ntr <- dim(as.matrix(index.list[[1]]))[2]
  }
  ...
  for (i in seq(1, nindex * 2, by = 2)) {
    res[[eval(namesindex.all[i])]] <- ses(
      obs = index.list[[i]],
      nullmodel = index.list[[i + 1]],
      val.quant = val.quant
    )
  }
  ...
  for (i in 1:nindex) {
    if (ntr[i] > 1) {
      nfactor <- c(nfactor, dim(as.matrix(res[[eval(namesindex[i])]]$ses))[1])
    }
    if (ntr[i] == 1) {
      nfactor <- c(nfactor, length(res[[eval(namesindex[i])]]$ses))
    }
  }
  ...
  if (type == "bytraits") {
    ...
    if (is.null(xlim)) {
      xlim = c(
        min(c(res.total, -2), na.rm = T),
        max(c(res.total, 2), na.rm = T)
      )
    }

    if (is.logical(color.cond)) {
      color.cond = c("blue", "orange")
    }

    if (bysite) {
      if (length(unique(ncom)) != 1) {
        stop("if type = 'bysites', all index need the same number of community")
      }

      for (s in 1:ncom[1]) {
        plot(
          mean(res[[eval(namesindex.all[1])]]$ses[s, ], na.rm = T),
          (1:nindex)[1],
          bty = "n",
          cex.lab = 0.8,
          yaxt = "n",
          xlab = paste("SES", namescommunity[s]),
          ylim = ylim,
          xlim = xlim,
          pch = 16,
          type = "n",
        ...
        for (i in 1:nindex) {
          ...
          segments(
            mean(res[[eval(namesindex[i])]]$ses.sup[s, ], na.rm = T),
            (1:nindex)[i],
            mean(res[[eval(namesindex[i])]]$ses.inf[s, ], na.rm = T),
            (1:nindex)[i]
          )
          ...
          points(
            res[[eval(namesindex[i])]]$ses[s, ],
            rep((1:nindex)[i], length(res[[eval(namesindex[i])]]$ses[s, ])),
            pch = "*"
          )

          cond.sup <- res[[eval(namesindex[i])]]$ses[s, ] >
            res[[eval(namesindex[i])]]$ses.sup[s, ]
          points(
            res[[eval(namesindex[i])]]$ses[s, ][cond.sup],
            rep(
              (1:nindex)[i],
              length(res[[eval(namesindex[i])]]$ses[s, ][cond.sup])
            ),
            pch = "*",
            cex = 3,
            col = color.cond[2]
          ...
          cond.inf <- res[[eval(namesindex[i])]]$ses[s, ] <
            res[[eval(namesindex[i])]]$ses.inf[s, ]
          points(
            res[[eval(namesindex[i])]]$ses[s, ][cond.inf],
            rep(
              (1:nindex)[i],
              length(res[[eval(namesindex[i])]]$ses[s, ][cond.inf])
            ),
            pch = "*",
            cex = 3,
            col = color.cond[1]
          ...
          points(
            mean(res[[eval(namesindex[i])]]$ses[s, ], na.rm = T),
            (1:nindex)[i],
            col = "red",
            pch = 16
          )
      ...
      for (t in 1:ntr[1]) {
        plot(
          mean(res[[eval(namesindex[i])]]$ses[, t], na.rm = T),
          (1:nindex)[i],
          bty = "n",
          cex.lab = 0.8,
          yaxt = "n",
          xlab = paste("SES", namestraits[t]),
          ylim = ylim,
          xlim = xlim,
          pch = 16,
          type = "n",
        ...
        for (i in 1:nindex) {
          ...
          segments(
            mean(res[[eval(namesindex[i])]]$ses.sup[, t], na.rm = T),
            (1:nindex)[i],
            mean(res[[eval(namesindex[i])]]$ses.inf[, t], na.rm = T),
            (1:nindex)[i]
          )
          ...
          points(
            res[[eval(namesindex[i])]]$ses[, t],
            rep((1:nindex)[i], length(res[[eval(namesindex[i])]]$ses[, t])),
            pch = "*"
          )

          cond.sup <- res[[eval(namesindex[i])]]$ses[, t] >
            res[[eval(namesindex[i])]]$ses.sup[, t]
          points(
            res[[eval(namesindex[i])]]$ses[, t][cond.sup],
            rep(
              (1:nindex)[i],
              length(res[[eval(namesindex[i])]]$ses[, t][cond.sup])
            ),
            pch = "*",
            cex = 3,
            col = color.cond[2]
          ...
          cond.inf <- res[[eval(namesindex[i])]]$ses[, t] <
            res[[eval(namesindex[i])]]$ses.inf[, t]
          points(
            res[[eval(namesindex[i])]]$ses[, t][cond.inf],
            rep(
              (1:nindex)[i],
              length(res[[eval(namesindex[i])]]$ses[, t][cond.inf])
            ),
            pch = "*",
            cex = 3,
            col = color.cond[1]
    ...
    try(axis(
      side = 2,
      seq(from = 5.5, to = 4.5 + (nindex + 1) * max(ntr), by = nindex + 1) +
        (nindex + 1) / 2,
      labels = namestraits,
      las = 1,
      cex.axis = 0.7
    ...
    if (grid.v == T) {
      range. <- max(c(res.total, 2), na.rm = T) -
        min(c(res.total, -2), na.rm = T)

      vect.grid <- seq(
        min(res.total, na.rm = T),
        max(res.total, na.rm = T),
        by = round(range., 2) / 9
      )
      for (j in vect.grid) {
        ...
        for (t in 1:ntr[i]) {
          if (add.conf == T) {
            ...
            polygon(
              xpolyg,
              ypolyg,
              col = transpa(col.index[i], alpha),
              border = transpa(col.index[i], alpha + (1 - alpha) / 2)
            )
          }
          ...
          if (color.cond == F) {
            points(
              res[[eval(namesindex[i])]]$ses[, t],
              rep(5.5 + (nindex + 1) * t - i, times = nfactor[i]),
              pch = 20,
              col = col.index[i]
            )
          }
          ...
          if (color.cond == T) {
            if (length(col.index) != 2 * nindex) {
              col.index[(nindex + 1):(nindex * 2)] <- "grey50"
            }
            points(
              res[[eval(namesindex[i])]]$ses[, t],
              rep(5.5 + (nindex + 1) * t - i, times = nfactor[i]),
              pch = 20,
              col = col.index[nindex + i]
            )
            condition <- res[[eval(namesindex[i])]]$ses[, t] >
              res[[eval(namesindex[i])]]$ses.sup[, t] |
              res[[eval(namesindex[i])]]$ses[, t] <
                res[[eval(namesindex[i])]]$ses.inf[, t]
            ...
            if (sum(condition) > 0) {
              points(
                res[[eval(namesindex[i])]]$ses[condition, t],
                rep(5.5 + (nindex + 1) * t - i, times = sum(condition)),
                pch = 20,
                col = col.index[i]
              )
            }
          ...
          abline(
            seq(from = 5.5, to = 4.5 + (nindex + 1) * ntr[i], by = nindex + 1)[
              t
            ],
            b = 0,
            lty = 4,
      ...
      for (i in 1:nindex) {
        for (t in 1:ntr[i]) {
          ...
          if (color.cond == T) {
            ...
            condition <- mean(res[[eval(namesindex[i])]]$ses[, t], na.rm = T) >
              mean(res[[eval(namesindex[i])]]$ses.sup[, t], na.rm = T) |
              mean(res[[eval(namesindex[i])]]$ses[, t], na.rm = T) <
                mean(res[[eval(namesindex[i])]]$ses.inf[, t], na.rm = T)
            if (sum(condition) > 0) {
          ...
          segments(
            mean(res[[eval(namesindex[i])]]$ses[, t], na.rm = T) +
              sd(res[[eval(namesindex[i])]]$ses[, t], na.rm = T),
            5.5 + (nindex + 1) * t - i,
            mean(res[[eval(namesindex[i])]]$ses[, t], na.rm = T) -
              sd(res[[eval(namesindex[i])]]$ses[, t], na.rm = T),
            5.5 + (nindex + 1) * t - i,
          ...
          points(
            min(res[[eval(namesindex[i])]]$ses[, t], na.rm = T),
            5.5 + (nindex + 1) * t - i,
            pch = "*",
            col = col.index[i]
          )
          points(
            max(res[[eval(namesindex[i])]]$ses[, t], na.rm = T),
            5.5 + (nindex + 1) * t - i,
            pch = "*",
            col = col.index[i]
          )
    ...
    print(paste("Error:", type, "is not a valid type of plot"))
  }
...
decompCTRE <- function(
  ...
  moy_pop <- apply(traits, 2, function(x) {
    tapply(x, paste(ind.plot, sp, sep = "."), mean, na.rm = T)
  })
  ...
  for (t in 1:ntr) {
    ...
    interm <- apply(comm, 2, function(x) {
      tapply(x, paste(ind.plot, sp, sep = "."), sum, na.rm = T)
    })[!is.na(moy_pop[, t]), 1:ncom]
    interm2 <- apply(comm, 2, function(x) tapply(x, sp, sum, na.rm = T))[
      !is.na(moy_sp[, t]),
      1:ncom
    ]

    specif.avg <- t(moy_pop[, t][!is.na(moy_pop[, t])] %*% interm) /
      colSums(interm)
    const.avg <- t(moy_sp[, t][!is.na(moy_sp[, t])] %*% interm2) /
      colSums(interm2)

    flex = paste(
      "traitflex.anova(",
      form.string,
      ", specif.avg, const.avg, ...)",
      sep = ""
    )
...
barplot.decompCTRE <- function(height, resume = TRUE, ...) {
  ...
  if (resume == TRUE) {
    res <- matrix(
      ncol = dim(as.matrix(x))[1] - 1,
      nrow = dim(x[[1]]$SumSq)[2] - 1
    )
    for (i in 1:(dim(as.matrix(x))[1] - 1)) {
      res[, i] <- plot.traitflex(
        x[[i]],
        plot.res = FALSE,
        cumul = TRUE,
        beside = T,
    ...
    legend(
      "topright",
      c("Turnover", "Intraspec.", "Covariation"),
      fill = c(gray.colors(3))
    )
...
traitflex.anova <- function(formula, specif.avg, const.avg, ...) {
  ...
  if (nrows > 1) {
    # get rid of the "Total" label again
...
plotCorTstats <- function(
  ...
  #________________________________________
  ses.T_IP.IC.moy <- t(colMeans(
    (Tst$T_IP.IC -
      apply(Tst$T_IP.IC_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
      apply(Tst$T_IP.IC_nm, c(3, 2), function(x) sd(x, na.rm = T)),
    na.rm = T
  ))
  ses.T_IC.IR.moy <- t(colMeans(
    (Tst$T_IC.IR -
      apply(Tst$T_IC.IR_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
      apply(Tst$T_IC.IR_nm, c(3, 2), function(x) sd(x, na.rm = T)),
    na.rm = T
  ))
  ses.T_PC.PR.moy <- t(colMeans(
    (Tst$T_PC.PR -
      apply(Tst$T_PC.PR_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
      apply(Tst$T_PC.PR_nm, c(3, 2), function(x) sd(x, na.rm = T)),
    na.rm = T
  ...
  ses.T_IP.IC.inf <- t(
    (apply(Tst$T_IP.IC_nm, c(3, 2), function(x) {
      quantile(x, na.rm = T, prob = val.quant[1])
    }) -
      apply(Tst$T_IP.IC_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
      apply(Tst$T_IP.IC_nm, c(3, 2), function(x) sd(x, na.rm = T))
  ...
  ses.T_IC.IR.inf <- t(
    (apply(Tst$T_IC.IR_nm, c(3, 2), function(x) {
      quantile(x, na.rm = T, prob = val.quant[1])
    }) -
      apply(Tst$T_IC.IR_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
      apply(Tst$T_IC.IR_nm, c(3, 2), function(x) sd(x, na.rm = T))
  ...
  ses.T_PC.PR.inf <- t(
    (apply(Tst$T_PC.PR_nm, c(3, 2), function(x) {
      quantile(x, na.rm = T, prob = val.quant[1])
    }) -
      apply(Tst$T_PC.PR_nm, c(3, 2), function(x) mean(x, na.rm = T))) /
      apply(Tst$T_PC.PR_nm, c(3, 2), function(x) sd(x, na.rm = T))
  ...
  if (bysite == F) {
    if (is.null(col.obj)) {
      col.obj <- funky.col(dim(Tst$T_IP.IC)[2])
    } else {}
    if (multipanel) {
      par(
        mfrow = c(sqrt(dim(Tst$T_IP.IC)[2]) + 1, sqrt(dim(Tst$T_IP.IC)[2]) + 1)
      )
    }
    ...
    for (t in 1:dim(Tst$T_IP.IC)[2]) {
      plot(
        as.vector(ses.T_IP.IC) ~ as.vector(ses.T_IC.IR),
        col = "grey",
        pch = 20,
        main = rownames(ses.T_IC.IR)[t],
        ...
      )
      points(
        sort(as.vector(ses.T_IP.IC)) ~ as.vector(
          ses.T_IC.IR.inf
        )[order(as.vector(ses.T_IP.IC))][!is.na(as.vector(ses.T_IP.IC))],
        type = "l"
      )
      points(
        sort(as.vector(ses.T_IP.IC)) ~ as.vector(
      ...
      points(
        sort(as.vector(ses.T_IC.IR)),
        as.vector(ses.T_IP.IC.inf)[order(as.vector(ses.T_IC.IR))][
          !is.na(as.vector(ses.T_IC.IR))
        ],
        type = "l"
      )
      points(
        sort(as.vector(ses.T_IC.IR)),
        as.vector(ses.T_IP.IC.sup)[order(as.vector(ses.T_IC.IR))][
          !is.na(as.vector(ses.T_IC.IR))
        ],
        type = "l"
      )
      segments(
        rep(rowMeans(ses.T_IC.IR, na.rm = T)[t], times = dim(Tst$T_IP.IC)[1]),
        rep(rowMeans(ses.T_IP.IC, na.rm = T)[t], times = dim(Tst$T_IP.IC)[1]),
        ses.T_IC.IR[t, ],
        ses.T_IP.IC[t, ],
        col = col.obj[t]
      )
    ...
    for (t in 1:dim(Tst$T_IP.IC)[2]) {
      plot(
        as.vector(ses.T_IP.IC) ~ as.vector(ses.T_PC.PR),
        col = "grey",
        pch = 20,
        main = rownames(ses.T_PC.PR)[t],
        ...
      )
      points(
        sort(as.vector(ses.T_IP.IC)) ~ as.vector(
      ...
      points(
        sort(as.vector(ses.T_PC.PR)),
        as.vector(ses.T_IP.IC.inf)[order(as.vector(ses.T_PC.PR))][
          !is.na(as.vector(ses.T_PC.PR))
        ],
        type = "l"
      )
      points(
        sort(as.vector(ses.T_PC.PR)),
        as.vector(ses.T_IP.IC.sup)[order(as.vector(ses.T_PC.PR))][
          !is.na(as.vector(ses.T_PC.PR))
        ],
        type = "l"
      )
      segments(
        rep(rowMeans(ses.T_PC.PR, na.rm = T)[t], times = dim(Tst$T_IP.IC)[1]),
        rep(rowMeans(ses.T_IP.IC, na.rm = T)[t], times = dim(Tst$T_IP.IC)[1]),
        ses.T_PC.PR[t, ],
        ses.T_IP.IC[t, ],
        col = col.obj[t]
    ...
    for (t in 1:dim(Tst$T_IC.IR)[2]) {
      plot(
        as.vector(ses.T_IC.IR) ~ as.vector(ses.T_PC.PR),
        col = "grey",
        pch = 20,
        main = rownames(ses.T_PC.PR)[t],
        ...
      ...
      points(
        sort(as.vector(ses.T_IC.IR)) ~ as.vector(
          ses.T_PC.PR.inf
        )[order(as.vector(ses.T_IC.IR))][!is.na(as.vector(ses.T_IC.IR))],
        type = "l"
      )
      points(
        sort(as.vector(ses.T_IC.IR)) ~ as.vector(
      ...
      points(
        sort(as.vector(ses.T_PC.PR)),
        as.vector(ses.T_IC.IR.inf)[order(as.vector(ses.T_PC.PR))][
          !is.na(as.vector(ses.T_PC.PR))
        ],
        type = "l"
      )
      points(
        sort(as.vector(ses.T_PC.PR)),
        as.vector(ses.T_IC.IR.sup)[order(as.vector(ses.T_PC.PR))][
          !is.na(as.vector(ses.T_PC.PR))
        ],
        type = "l"
      )
      segments(
        rep(rowMeans(ses.T_PC.PR, na.rm = T)[t], times = dim(Tst$T_IC.IR)[1]),
        rep(rowMeans(ses.T_IC.IR, na.rm = T)[t], times = dim(Tst$T_IC.IR)[1]),
        ses.T_PC.PR[t, ],
        ses.T_IC.IR[t, ],
        col = col.obj[t]
      )
    ...
    #________________________________________
    if (is.null(col.obj)) {
      col.obj <- funky.col(dim(Tst$T_IP.IC)[1])
    } else {}

    if (multipanel) {
      par(
        mfrow = c(sqrt(dim(Tst$T_IP.IC)[1]) + 1, sqrt(dim(Tst$T_IP.IC)[1]) + 1)
      )
    }
    ...
    for (s in 1:dim(Tst$T_IP.IC)[1]) {
      ...
      segments(
        rep(colMeans(ses.T_IC.IR, na.rm = T)[s], times = dim(Tst$T_IP.IC)[2]),
        rep(colMeans(ses.T_IP.IC, na.rm = T)[s], times = dim(Tst$T_IP.IC)[2]),
        ses.T_IC.IR[, s],
        ses.T_IP.IC[, s],
        col = col.obj[s]
      )
    ...
    for (s in 1:dim(Tst$T_IP.IC)[1]) {
      ...
      segments(
        rep(colMeans(ses.T_PC.PR, na.rm = T)[s], times = dim(Tst$T_IP.IC)[2]),
        rep(colMeans(ses.T_IP.IC, na.rm = T)[s], times = dim(Tst$T_IP.IC)[2]),
        ses.T_PC.PR[, s],
        ses.T_IP.IC[, s],
        col = col.obj[s]
    ...
    for (s in 1:dim(Tst$T_IC.IR)[1]) {
      ...
      segments(
        rep(colMeans(ses.T_PC.PR, na.rm = T)[s], times = dim(Tst$T_IC.IR)[2]),
        rep(colMeans(ses.T_IC.IR, na.rm = T)[s], times = dim(Tst$T_IC.IR)[2]),
        ses.T_PC.PR[, s],
        ses.T_IC.IR[, s],
        col = col.obj[s]
      )
    ...
    print("Error: obj need to be either traits or sites")
  }
...
plotSESvar <- function(
  ...
  for (i in seq(1, nindex * 2, by = 2)) {
    if (resume == FALSE) {
      xlim = c(
        min(c(-4, res[[eval(namesindex.all[i])]]$ses), na.rm = T),
        max(c(4, res[[eval(namesindex.all[i])]]$ses), na.rm = T)
      )
    } else {
      xlim = c(
        min(
          c(
            -4,
            rowMeans(res[[eval(namesindex.all[i])]]$ses, na.rm = T) -
              apply(res[[eval(namesindex.all[i])]]$ses, 1, sd, na.rm = T),
            rowMeans(res[[eval(namesindex.all[i])]]$ses, na.rm = T) +
              apply(res[[eval(namesindex.all[i])]]$ses, 1, sd, na.rm = T)
          ),
          na.rm = T
    ...
    plot(
      ...
      bty = "n",
      cex.lab = 0.8,
      xlab = paste("SES", namesindex.all[i]),
      ylab = ylab,
      ylim = ylim,
      xlim = xlim,
      pch = 16,
    ...
    if (resume == TRUE) {
      ...
      for (t in 1:ntr) {
        points(
          res[[eval(namesindex.all[i])]]$ses[, t],
          y,
          pch = 16,
          col = color.traits[t]
        )
  ...
  if (resume != TRUE) {
    ...
    legend(
      ...
      fill = color.traits,
      bty = "n",
      ncol = round(sqrt(nlevels(as.factor(namestraits))) - 1)
    )
  }
...
plotDistri <- function(
  ...
  #Graphical parameters
  if (is.null(col.dens)) {
    col.dens <- funky.col(nlevels(as.factor(var.2)))
  }
  if (length(leg) != ntr) {
    leg <- rep_len(leg, ntr)
  }
  ...
  for (co in 1:ncom) {
    for (t in 1:ntr) {
      ...
      if (
        length(na.omit(traits[
          as.factor(var.1) == levels(as.factor(var.1))[co],
          t
        ])) >
          1
      ) {
        ...
        for (s in 1:nlevels(as.factor(var.2))) {
          if (
            length(na.omit(traits[
              as.factor(var.1) == levels(as.factor(var.1))[co] &
                as.factor(var.2) == levels(as.factor(var.2))[s],
              t
            ])) >
              1
          ) {
        ...
        if (is.null(xlim)) {
          ...
          xlimite = c(
            min(c(min(density(traits[, t], na.rm = T)$x), xlimin)),
            max(c(max(density(traits[, t], na.rm = T)$x, na.rm = T), xli))
          )
        } else {
          xlimite <- xlim
        }
        ...
        if (is.null(ylim)) {
          ...
          ylimite = c(
            min(c(min(density(traits[, t], na.rm = T)$y), ylimin)),
            max(c(max(density(traits[, t], na.rm = T)$y, na.rm = T), yli)) *
              1.05
          )
        } else {
          ylimite <- ylim
        ...
        plot(
          main = main.plot,
          density(
            traits[as.factor(var.1) == levels(as.factor(var.1))[co], t],
            na.rm = T
          ),
          ylim = ylimite,
          xlim = xlimite,
        ...
        if (leg[t]) {
          ...
          legend(
            pos,
            inset = 0.05,
            levels(as.factor(var.2)),
            fill = col.dens,
            cex = cex.leg,
            bty = "n",
            ncol = round(sqrt(nlevels(as.factor((var.2)))) - 1)
          )
        }
...
plotSpPop <- function(
  ...
  min.ind.signif = 10,
  multipanel = TRUE,
  col.nonsignif.lm = rgb(0, 0, 0, 0.5),
  col.signif.lm = rgb(1, 0.1, 0.1, 0.8),
  silent = FALSE
) {
  ntr <- dim(traits)[2]
  namestraits <- colnames(traits)
  ...
  plotsp = unlist(strsplit(levels(as.factor(name_sp_sites)), split = "_"))[seq(
    3,
    3 * nlevels(as.factor(name_sp_sites)),
    by = 3
  )]
  #plosp is the plot in wich the population is
  plotind = unlist(strsplit(name_sp_sites, split = "_"))[seq(
    3,
    3 * length(name_sp_sites),
    by = 3
  )]
  spplot = paste(
    unlist(strsplit(levels(as.factor(name_sp_sites)), split = "_"))[seq(
      1,
      3 * nlevels(as.factor(name_sp_sites)),
      by = 3
    )],
  ...
  traits_by_sites <- apply(traits, 2, function(x) {
    tapply(x, ind.plot, mean, na.rm = T)
  })
  ...
  if (!resume) {
    for (t in 1:ntr) {
      ...
      for (s in 1:nlevels(sp)) {
        try(
          interm <- lm(
            y.pop[spplot == levels(sp)[s]] ~ x.pop[spplot == levels(sp)[s]]
          ),
          silent = silent
        )
        ...
        if (
          ...
          if (sum(!is.na(summary(interm)$coefficient[, 4])) > 0) {
            if (
              summary(interm)$coefficients[2, 4] < p.val &
                length(interm$fitted.values) > min.ind.signif
            ) {
              lty.lm = 1
              lwd.lm = 3
      ...
      try(
        interm2 <- lm(
          traits_by_sites[
            match(plotsp, rownames(traits_by_sites)),
            t
          ] ~ traits_by_pop[, t]
        ),
        silent = silent
      ...
      if (
        ...
        if (sum(!is.na(summary(interm2)$coefficient[, 4]) > 0)) {
          if (
            summary(interm2)$coefficients[2, 4] < p.val &
              length(interm2$fitted.values) > min.ind.signif
          ) {
      ...
      points(
        tapply(traits_by_pop[, t], plotsp, mean, na.rm = T) ~ tapply(
          traits_by_sites[match(plotsp, rownames(traits_by_sites)), t],
          plotsp,
          mean,
          na.rm = T
        ),
      ...
      points(
        tapply(traits_by_pop[, t], spplot, mean, na.rm = T) ~ tapply(
          traits_by_sites[match(plotsp, rownames(traits_by_sites)), t],
          spplot,
          mean,
          na.rm = T
...
plotSpVar <- function(
  ...
  interm.for.names <- apply(traits, 2, function(x) {
    tapply(x, ind.plot, mean, na.rm = T)
  })
...
ses <- function(obs = NULL, nullmodel = NULL, val.quant = c(0.025, 0.975)) {
  ...
  if (dim(obs)[1] == dim(obs)[2]) {
    ...
    if (is.list(nullmodel)) {
      if (is.list(nullmodel[[1]])) {
        nullmodel <- array(
          unlist(nullmodel, use.names = FALSE),
          dim = c(
            nrow(nullmodel[[1]][[1]]),
            ncol(nullmodel[[1]][[1]]),
            length(unlist(nullmodel, use.names = FALSE)) /
              nrow(nullmodel[[1]][[1]]) /
              ncol(nullmodel[[1]][[1]])
          )
        )
    ...
    if (
      !is.null(nullmodel) &&
        length(dim(nullmodel)) == 3 &&
        dim(nullmodel)[1] != dim(obs)[1]
    ) {
      cond <- c(2, 3)
    } else {
      cond <- c(1, 2)
  ...
  res <- list()
  res$ses <- (obs - apply(nullmodel, cond, function(x) mean(x, na.rm = T))) /
    apply(nullmodel, cond, function(x) sd(x, na.rm = T))
  res$ses.inf <- (apply(nullmodel, cond, function(x) {
    quantile(x, na.rm = T, prob = val.quant[1])
  }) -
    apply(nullmodel, cond, function(x) mean(x, na.rm = T))) /
    apply(nullmodel, cond, function(x) sd(x, na.rm = T))
  res$ses.sup <- (apply(nullmodel, cond, function(x) {
    quantile(x, na.rm = T, prob = val.quant[2])
  }) -
    apply(nullmodel, cond, function(x) mean(x, na.rm = T))) /
    apply(nullmodel, cond, function(x) sd(x, na.rm = T))
...
RaoRel <- function(
  ...
  Qdecomp = function(functdist, abundances, w = TRUE) {
    ...
    if (dim(abundances)[2] != dim(functdist)[1]) {
      stop(
        "error : different number of species in 'functdist' and 'abundances' "
      )
    }
    ...
    if (is.na(sum(abundances)) == T) {
      for (i in 1:dim(abundances)[1]) {
        for (j in 1:dim(abundances)[2]) {
          if (is.na(abundances[i, j]) == T) abundances[i, j] <- 0
        }
      } # end of i j
    } # end of if
  ...
  disc = function(samples, dis = NULL, structures = NULL, Jost = F) {
    ...
    if (!is.null(structures)) {
      ...
      m <- match(
        tapply(1:ncol(structures), as.factor(1:ncol(structures)), function(x) {
...
plotRandtest <- function(x, alternative = "two-sided", ...) {
  ...
  if (!inherits(x, "listofindex")) {
    if (
      inherits(x, "Tstats") |
        inherits(x, "ComIndex") |
        inherits(x, "ComIndexMulti")
    ) {
      x <- as.listofindex(x)
    } else {
      stop(
        "x must be a list of objects of class listofindex, Tstats, ComIndex or ComIndexMulti"
      )
    }
  }
  ...
  for (i in seq(1, nindex * 2, by = 2)) {
    for (t in 1:ntr[1]) {
      for (s in 1:ncom[1]) {
        rt <- as.randtest(
          sim = na.omit(index.list[[i + 1]][, t, s]),
          obs = na.omit(index.list[[i]][s, t]),
          alter = alternative
        )
        plot(
          rt,
...
AbToInd <- function(traits, com, type.sp.val = "count") {
  ...
  if (type.sp.val == "abundance") {
    if (min(com) < 1) {
      com <- apply(com, 2, function(x) {
        round(x / min(x[x > 0], na.rm = T), 1) * 10
      })
    }
  }
...
CVNND <- function(
  ...
  #Metric calculation
  CVNND <- sd(nnd[is.finite(nnd)], na.rm = na.rm) /
    mean(nnd[is.finite(nnd)], na.rm = na.rm)
...
SumBL <- function(
  ...
  if (gower.dist) {
    tree.dist <- cluster::daisy(traits, metric = "gower")
  } else {
    if (scale.tr) {
      traits <- apply(traits, 2, scale)
    }
...
IndexByGroups <- function(metrics, groups) {
  ...
  for (i in 1:length(metrics)) {
    res <- c(
      res,
      paste("tapply(x, ", groups, ", function(x) ", metrics[i], ")", sep = "")
    )
  }
...
Fred <- function(traits, ind.plot = NULL) {
  if (is.null(ind.plot)) {
    ...
    warnings(
      "The argument `ind.plot` is null. Only one value will be compute for each metrics."
    )
  }
  ...
  if (dim(abundances)[2] != dim(traits)[1]) {
    stop(
      " error : different number of individuals in 'traits' and in 'ind.plot' "
    )
  }

  # check absence of NA in 'traits'
  if (length(which(is.na(traits) == T)) != 0) {
    stop(" error : NA in 'traits' matrix ")
  }
  ...
  nbsp <- rep(NA, C)
  names(nbsp) <- row.names(abundances)
  FRic <- rep(NA, C)
  names(FRic) <- row.names(abundances)
  FEve <- rep(NA, C)
  names(FEve) <- row.names(abundances)
  FDiv <- rep(NA, C)
  names(FDiv) <- row.names(abundances)
  ...
  for (i in 1:C) {
    ...
    # check if
    if (S <= T) {
      stop(" number of individuals must be higher than number of traits ")
    }
  ...
  for (i in seq(1, length(x), 2)) {
    ...
    for (t in 1:length(eval(ni_tot))) {
      ...
      for (g in 1:length(obs[, t])) {
        res[[eval(names(x[i]))]][[eval(ni)]]$pval.inf[g] <- (1 +
          sum(obs[g, t] < nullmodel[g, t, ], na.rm = T)) /
          (dim(nullmodel)[3] + 1)
        res[[eval(names(x[i]))]][[eval(ni)]]$pval.sup[g] <- (1 +
          sum(obs[g, t] > nullmodel[g, t, ], na.rm = T)) /
          (dim(nullmodel)[3] + 1)
      }

      if (!is.null(dimnames(x[[1]])[[1]])) {
        res[[eval(names(x[i]))]][[eval(ni)]]$pval.inf <- cbind(
          res[[eval(names(x[i]))]][[eval(ni)]]$pval.inf
        )
        rownames(res[[eval(names(x[i]))]][[eval(ni)]]$pval.inf) <- dimnames(x[[
          1
        ]])[[1]]

        res[[eval(names(x[i]))]][[eval(ni)]]$pval.sup <- cbind(
          res[[eval(names(x[i]))]][[eval(ni)]]$pval.sup
        )
        rownames(res[[eval(names(x[i]))]][[eval(ni)]]$pval.sup) <- dimnames(x[[
          1
        ]])[[1]]
      }

      if (!is.null(dimnames(x[[1]])[[2]][t])) {
        colnames(res[[eval(names(x[i]))]][[eval(ni)]]$pval.sup) <- dimnames(x[[
          1
        ]])[[2]][t]
        colnames(res[[eval(names(x[i]))]][[eval(ni)]]$pval.inf) <- dimnames(x[[
          1
        ]])[[2]][t]
      }
    }
...
RandCom <- function(
  ...
  sdlog = 1.5,
  min_value_traits = 80,
  max_value_traits = 200,
  cv_intra_sp = 1.5,
  cv_intra_com = 1.5,
  Int_Filter_Strength = 50,
  Ext_Filter_Strength = 50,
  Filter = "None"
) {
  ...
  Data <- data.frame(matrix(0, ncol = 2 + Ntr, nrow = Nind))
  colnames(Data) = c("com", "sp", "trait1", "trait2")
  ...
  if (Filter == 'None') {
    ...
    Data$trait1 = rnorm(
      Nind,
      (max_value_traits - min_value_traits),
      (max_value_traits - min_value_traits) * cv_intra_sp
    )
  ...
  if (Filter == 'Internal') {
    ...
    Init_sp_mean <- round(
      sort(seq(min_value_traits, max_value_traits, length.out = Nsp)),
      2
    )
    ...
    for (i in 1:Nind) {
      ...
      Data$trait1[i] = rnorm(
        1,
        mean.sp[which(Data$sp[i] == sp)],
        abs(mean.sp[which(Data$sp[i] == sp)]) * cv_intra_sp
      )
      #trait 2 : uniform distribution
      Data$trait2[i] = runif(
        1,
        mean.sp[which(Data$sp[i] == sp)] * (1 - cv_intra_sp),
        abs(mean.sp[which(Data$sp[i] == sp)]) * (1 + cv_intra_sp)
      )
    }
  }
  ...
  if (Filter == 'External') {
    ...
    Init_com_mean <- round(
      sort(seq(min_value_traits, max_value_traits, length.out = Ncom)),
      2
    )
    ...
    for (i in 1:Nind) {
      ...
      Data$trait1[i] = rnorm(
        1,
        mean.com[which(Data$com[i] == com)],
        abs(mean.com[which(Data$com[i] == com)]) * cv_intra_com
      )
      #trait 2 : uniform distribution
      Data$trait2[i] = runif(
        1,
        mean.com[which(Data$com[i] == com)] * (1 - cv_intra_com),
        abs(mean.com[which(Data$com[i] == com)]) * (1 + cv_intra_com)
      )
    }
  }
...
#' @export
samplingSubsetData <- function(
  d = NULL,
  sampUnit = NULL,
  nperm = 9,
  type = "proportion",
  prop = seq(10, 100, by = 10),
  MinSample = 1,
  Size = NULL
) {
  type <- match.arg(
    type,
    c("proportion", "count", "factorBySize", "propBySize")
  )
    ...
    if (type == "propBySize") {
      ...
      for (j in prop) {
        ...
        for (i in levels(sampUnit)) {
          ...
          d.intern[[j]][[i]] <- d[sampUnit == i, ][
            order(Size[sampUnit == i], decreasing = TRUE),
          ][1:jj, ]
        }
        subsets_interm[[j]] <- lapply(
          d.intern[lapply(d.intern, length) > 0],
          function(x) do.call(rbind, x)
        )
      }
      subsets[[np]] <- subsets_interm[[j]]
    } else if (
      type == "proportion" | type == "count" | type == "factorBySize"
    ) {
      stock <- list()
        ...
        for (i in levels(sampUnit)) {
          ...
          if (type == "proportion") {
            ...
            if (jj != 0) {
              ...
              stock[[j]][[i]] <- c(toSample[sample.int(
                length(toSample),
                size = jj
              )])
            } else {
